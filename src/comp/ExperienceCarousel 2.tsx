...
const MOBILE_BULLETS = 2; // ponytail: was a design-tool slider (1-4); bullets shown before "Show all N" on phone
...
function computeGeo(stageW: number, innerH: number): Geo {
  ...
  const lift = narrow ? Math.round(cardH * 0.15) : 0; // phone: how far side cards rise above the front card
  const sSide = PERSPECTIVE / (PERSPECTIVE + R);
  const sideTop = (narrow ? T - lift : 0) - (sSide * cardH) / 2;
  const top = Math.min(-G / 2, sideTop);
  const bottom = T + cardH / 2;
  const margin = narrow ? 16 : 24;
  // Max depth throw for the card furthest from the viewer (u=1). Desktop
  // anchors it to G (the globe's reserved diameter) on top of the orbit's
  // own radius; phone drops the G term, matching the Orbit v6 Phone spec.
  const Z = narrow ? 2 * R : G + 2 * R;
  return {
    cardW,
...
const ExperienceCarousel: React.FC<ExperienceCarouselProps> = ({ items }) => {
  ...
  const dragRef = useRef<{ id: number; x0: number; pos0: number; moved: boolean; k: number | null; expand: boolean } | null>(
    null
  );
  ...
  const expandAnimRef = useRef<number | undefined>(undefined);
  const clickGuardRef = useRef(0);
  useEffect(() => () => window.clearTimeout(expandAnimRef.current), []);
  ...
  // Phone-only: bullets collapse to MOBILE_BULLETS by default, shared across
  // all cards. `expandAnim` turns on a 500ms height transition just for the
  // toggle window so a live resize elsewhere doesn't animate.
  const toggleExpanded = useCallback(() => {
    pause(6000);
    window.clearTimeout(expandAnimRef.current);
    setExpanded((v) => !v);
    setExpandAnim(true);
    expandAnimRef.current = window.setTimeout(() => setExpandAnim(false), 600);
  }, [pause]);
  ...
  const onDown = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const hit = target.closest?.("[data-k]") as HTMLElement | null;
    dragRef.current = {
      id: e.pointerId,
      x0: e.clientX,
      ...
      expand: !!target.closest?.("[data-expand]"),
    };
    try {
  ...
  const onUp = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (!d.moved) {
        if (d.expand) {
          clickGuardRef.current = performance.now() + 500;
          toggleExpanded();
          return;
        }
    ...
    [pause, go, count, setRing, toggleExpanded]
  );
  ...
  const colorTransition = expandAnim ? `${COLOR_TRANSITION},height 500ms ${EASE}` : COLOR_TRANSITION;
  const transition =
    s.dragging || s.spinning
      ? colorTransition
      : `transform ${duration}ms ${EASE},opacity ${duration}ms ${EASE},${colorTransition}`;
    ...
    <div ref={wrapRef} onKeyDown={onKey} className="relative w-full max-w-5xl mx-auto px-4 flex flex-col gap-10">
      <div
        ...
        style={{
          ...
          transition: expandAnim ? `height 500ms ${EASE}` : "none",
        }}
      >
        <div className="absolute left-0 right-0 h-0" style={{ top: geo.sceneTop }}>
          ...
          {cards.map((item, k) => {
            ...
            const x = geo.R * sinT;
            const z = -geo.Z * u;
            const y = geo.narrow ? geo.T - geo.lift * (1 - cosT) : geo.T * cosT;
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    {geo.narrow && item.bullets.length > MOBILE_BULLETS && (
                      <button
                        type="button"
                        data-expand="1"
                        aria-expanded={expanded}
                        onClick={() => {
                          if (performance.now() < clickGuardRef.current) return;
                          toggleExpanded();
                        }}
                        className="mt-1 self-start inline-flex items-center gap-1.5 min-h-11 cursor-pointer bg-transparent border-0 p-0 text-sm font-bold tracking-wide text-blue-400 transition-colors duration-300 hover:text-indigo-200"
                      >
                        <span>{expanded ? "Show less" : `Show all ${item.bullets.length}`}</span>
                        <svg
                          aria-hidden="true"
                          width="16"
                          height="16"
          ...
          {cards.map((item, k) => {
            ...
            return (
              <button
                ...
                aria-current={on}
                onClick={() => go(k)}
                className={`cursor-pointer min-w-11 min-h-11 rounded-full flex items-center justify-center gap-2.5 transition-all duration-500 border focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-400 focus-visible:outline-offset-2 ${
                  geo.narrow ? "" : "px-4"
                }`}
                style={{
                  background: geo.narrow ? "transparent" : on ? "rgba(0,0,0,.6)" : "rgba(0,0,0,.4)",
                  borderColor: geo.narrow ? "transparent" : on ? "rgba(59,130,246,.4)" : "rgba(255,255,255,.1)",
                  boxShadow: !geo.narrow && on ? "0 0 15px rgba(59,130,246,.35),0 0 30px rgba(99,102,241,.2)" : "none",
                }}
              >
                <span
                  aria-hidden="true"
                  className={`rounded-full transition-all duration-500 ${geo.narrow ? "w-2.5 h-2.5" : "w-2 h-2"}`}
                  style={{
                ...
                {!geo.narrow && (
                  <span
                    className="text-xs font-bold tracking-wider tabular-nums transition-colors duration-500"
